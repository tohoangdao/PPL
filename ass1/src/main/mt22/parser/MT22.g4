...
decls: vardec | funcdec;

vardec: (var_assign | var_declare) SEMICO;
var_assign: ID COMMA var_assign COMMA expr | ID COLON type_list ASSIGN expr;
var_declare: id_list COLON type_list;
id_list: ID (COMMA ID)?;

funcdec: ID COLON FUNCTION type_list OPENCC param_list? CLOSECC (INHERIT ID)? funcbody ;
funcbody:block_statement;
param: INHERIT? OUT? ID COLON type_list;
