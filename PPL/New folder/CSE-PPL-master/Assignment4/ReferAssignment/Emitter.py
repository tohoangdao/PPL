...
class Emitter():
    ...
    *
    ...
    def emitNEWARRAY(self, size, typ, idx, frame):
        buffer = list()
        buffer.append(self.emitPUSHICONST(size,frame))
        if type(typ) is ClassType or type(typ) is StringType:
            buffer.append(self.jvm.emitANEWARRAY(self.getFullType(typ)))
        else: buffer.append(self.jvm.emitNEWARRAY(self.getFullType(typ)))
        return ''.join(buffer)
    ...
    *   @param isFinal true in case of constant; false otherwise
    '''
    def emitATTRIBUTE(self, lexeme, in_, isFinal, value, isStatic):
        #lexeme: String
        #in_: Type
        #isFinal: Boolean
        #value: String
    ...
    def emitNEW(self,lexeme,in_,frame):
        ...
        frame.push()
        return self.jvm.emitNEW(self.getFullType(lexeme))
    ...
    *   @param in the type descriptor of the method.
    ...
    def emitINVOKESPECIAL(self, frame, lexeme=None, in_=None):
        ...
        if not lexeme is None and not in_ is None:
            ...
            if not type(typ.rettype) is VoidType:
                frame.push()
            return self.jvm.emitINVOKESPECIAL(self.getFullType(lexeme)+'/<init>', self.getJVMType(in_))
