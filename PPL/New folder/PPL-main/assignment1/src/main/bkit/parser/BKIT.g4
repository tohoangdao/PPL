...
varlist: var (ASSIGN literal)? (COMMA varlist)* ;
var: scalar|composite ;
scalar: ID;
composite: ID (LS INTLIT RS)+;

// Function declaration part
funcdecl: FUNCTION COLON ID (PARAMETER COLON paralist)? body ;
paralist: para (COMMA para)* ;
para: var (COMMA var)* ;
body: BODY COLON vardecl* stmt ENDBODY DOT ;
...
//expr1: <assoc=right> expr1 @ expr2 | expr2;
//expr2: expr2 ^ expr3 | expr3;
//expr3: expr4 ? expre3 | expr4;
//expr4: ( expr1 ) | INT ;
...
expr7: funccall | literal | LR expression RR | ID;


// Index operator
element_expr: (ID|funccall) index_op ;
funccall: ID LR exprlist? RR ;
index_op: LS expression RS | LS expression RS index_op ;


stmt: vardeclstmt* (assignstmt|ifstmt|forstmt|whilestmt|dowhilestmt|breakstmt|continuestmt|callstmt|returnstmt)* ;

// Variable declaration statement
vardeclstmt: vardecl;

// Assignment
assignstmt: scalar index_op? ASSIGN expression SEMI;

// If Statement
ifstmt: IF expression THEN stmt (ELSEIF expression THEN stmt)* (ELSE stmt)? ENDIF DOT ;

// For statement
forstmt: FOR LR scalar ASSIGN initExpr COMMA conditionExpr COMMA updateExpr RR DO stmt ENDFOR DOT ;
initExpr: expression;
conditionExpr: expression;
updateExpr: expression;

// While statement
whilestmt: WHILE expression DO stmt ENDWHILE DOT;
